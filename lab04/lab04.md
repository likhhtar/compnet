...
3. На какой IP-адрес отправлен DNS-запрос? Используйте ipconfig для определения IP-адреса
   ...
   - Да <img src="img/14.png" width=500>
...
5. Проанализируйте ответное сообщение DNS. Сколько в нем «ответов»? Что содержится в
   каждом?
   - 2 ответа
   - TTL - время, после которого кэшированный ответ станет недействительным; Тип записи; размер данных; ip адрес и имя хоста
...
7. Веб-страница содержит изображения. Выполняет ли хост новые запросы DNS перед
   ...
   - Да, выполняется запрос к адресу static.ietf.org
...
<img src="img/17.png" width=500>
   - 2
   - В первом каноничное имя, во втором пара (spbu.ru, 85.193.83.151)
...
1. Что такое база данных whois?
   - база данных, в которой хранится информация о владельцах IP адресов и некоторая другая информация (статус и тд)
2. Используя различные сервисы whois в Интернете, получите имена любых двух DNS-серверов. 
   ...
   - NS1.VK.COM, NS2.VK.COM, NS3.VK.COM, NS4.VK.COM
   - https://whois.ru/
