...
def run_rip(routers: Dict[str, Router]):
    ...
    while changed:
        ...
        # Выводим таблицу после каждой итерации
        for router in routers.values():
            print(f"\nSimulation step {step} of router {router.ip}")
            print(f"{'Source IP':<16} {'Destination IP':<16} {'Next Hop':<16} {'Metric':<6}")
            for dest_ip, (next_hop, metric) in sorted(router.routing_table.items()):
                print(f"{router.ip:<16} {dest_ip:<16} {next_hop:<16} {metric:<6}")
